...
project = pd.read_csv('HP2.csv',dtype=str,sep='\t')
...
for index,row in user.iterrows():
  ...
  project_score.append ( GetGrade ( row['status'], 0, float(row['scores_table.snp_score']), float(row['scores_table.indel_score']), 0, 0, 0 ) [1] * 100 )
...
user.to_csv('HP2Score.csv',index=False,sep='\t')
